...
# output_dir
parser.add_argument('--output_dir', default='data/vizwizbu_taiyin_full', help='output feature files')
...
for infile in infiles:
    ...
    with open(os.path.join('/home/yz9244/Up-Down-Captioner/bottom-up-attention/tsv_taiyin_full/', infile), "r+b") as tsv_in_file:
        reader = csv.DictReader(tsv_in_file, delimiter='\t', fieldnames = FIELDNAMES)
        for item in reader:
            item['image_id'] = int(item['image_id'])
            item['num_boxes'] = int(item['num_boxes'])
            #print("num boxes: %d" % (item['num_boxes']))
            #print(np.frombuffer(base64.b64decode(item['boxes']+"==="),dtype=np.float32).shape)
            #print(len(base64.b64decode(item['features']+"===")))
            #print(np.frombuffer(base64.decodestring(item['features']+"==="), dtype=np.float32).shape)
